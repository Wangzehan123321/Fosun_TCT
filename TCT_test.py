...
#model.load_state_dict(torch.load('../TCT_trainedmodels/resnet18_focial/cslstm_99.tar'))
#model.load_state_dict(torch.load('../TCT_trainedmodels/resnet18_augment_focial/cslstm_35.tar'))
model.load_state_dict(torch.load('../TCT_trainedmodels/resnet18_augment_balance_focial/cslstm_35.tar'))

#定义测试集
from TCT_dataset import TCTDataset
batch_size=64
num_workers=8
tsSet = TCTDataset("/home/wangzehan/TCT_data/data_process/val", train=False)
testDataloader = DataLoader(tsSet, batch_size,shuffle=True,num_workers=num_workers)

#TODO:在测试的时候考虑使用多尺度测试
